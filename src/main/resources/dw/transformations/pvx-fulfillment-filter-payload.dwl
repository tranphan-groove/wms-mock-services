...
var transformedPayload = flatten((vars.salesOrders filter ((item) -> (
    ...
    (isEmpty(vars.searchClauseTimestamps[1]) or ((item.timestamp as DateTime) < (vars.searchClauseTimestamps[1] as DateTime)))
))) map ((order) -> (vars.salesOrderItems filter ((item) -> item.SalesOrderNumber == order.SalesOrderNumber)) map ((item) -> {
      ...
      "Shipping address country": order.InvoiceAddressCountry default "",
      ...
      "Despatch carrier name": order.ServiceType default "",
      "Despatch service type code": order.Attribute1 default "",
      "Package carrier name": order.ServiceType default "",
      "Package service type code": order.Attribute1 default "",
